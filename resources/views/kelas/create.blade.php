                ...
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('kelas.store') }}" class="space-y-6">
                        ...
                        <div>
                            ...
                            <x-text-input id="kode_kelas" class="block mt-1 w-full" type="text" name="kode_kelas" :value="old('kode_kelas')" required autofocus placeholder="Contoh: TI-1A" />
                            <x-input-error :messages="$errors->get('kode_kelas')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="nama_kelas" :value="__('Nama Kelas')" />
                            <x-text-input id="nama_kelas" class="block mt-1 w-full" type="text" name="nama_kelas" :value="old('nama_kelas')" required placeholder="Contoh: Teknik Informatika Kelas A" />
                            <x-input-error :messages="$errors->get('nama_kelas')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="kapasitas" :value="__('Kapasitas Mahasiswa')" />
                            <x-text-input id="kapasitas" class="block mt-1 w-full" type="number" name="kapasitas" :value="old('kapasitas', 30)" required />
